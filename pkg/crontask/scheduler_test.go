...
func TestRegisterAndRemove(t *testing.T) {
	...
	if err := s.Remove(id); !errors.Is(err, ErrJobNotFound) {
...
// TestNextRuns verifies deterministic next-run calculation.
...
// TestNextRunsNotFound verifies that NextRuns returns ErrJobNotFound for an
// unknown ID.
func TestNextRunsNotFound(t *testing.T) {
	t.Parallel()
	s, _ := New()
...
func TestWithErrorHandler(t *testing.T) {
	...
	s, _ := New(
		WithSeconds(),
		WithErrorHandler(func(_ string, _ error) {
...
func TestWithLocation(t *testing.T) {
	...
	s, err := New(WithLocation(tz))
	if err != nil {
		t.Fatalf("New(): %v", err)
	}
	if s.cfg.loc != tz {
		t.Errorf("expected location %v, got %v", tz, s.cfg.loc)
	}
}
...
func TestBackoffPolicies(t *testing.T) {
	...
	cb := ConstantBackoff(5 * time.Second)
	for i := 1; i <= 5; i++ {
	...
	eb := ExponentialBackoff(time.Second)
...
func TestJobErrorUnwrap(t *testing.T) {
	...
	je := &JobError{JobID: "x", Attempt: 1, Err: inner}
...
func TestSchedulerWithSecondsOption(t *testing.T) {
	...
	s, _ := New(WithSeconds())
	if !s.cfg.withSecs {
		t.Error("withSecs should be true after WithSeconds()")
	}
}
...
func (h *testHooks) OnSuccess(_ context.Context, id string, d time.Duration) {
	if h.onSuccess != nil {
		h.onSuccess("", id, d)
	}
}

func (h *testHooks) OnFailure(_ context.Context, id string, d time.Duration, err error) {
	if h.onFailure != nil {
		h.onFailure("", id, d, err)
	}
}

func (h *testHooks) OnComplete(_ context.Context, id string, d time.Duration) {
	if h.onComplete != nil {
		h.onComplete("", id, d)
	}
}
